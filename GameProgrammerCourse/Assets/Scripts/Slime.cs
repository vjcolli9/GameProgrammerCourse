...
public class Slime : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_direction < 0)
        {
            ...
            RaycastHit2D result = Physics2D.Raycast(_leftSensor.position, Vector2.down, 0.1f);
            RaycastHit2D wallResult = Physics2D.Raycast(_leftSensor.position, Vector2.left, 0.1f);

            if (result.collider == null)
                TurnAround();

            if (wallResult.collider != null)
                TurnAround();
        }
        else
        {
            ...
            RaycastHit2D wallResult = Physics2D.Raycast(_rightSensor.position, Vector2.right, 0.1f);
